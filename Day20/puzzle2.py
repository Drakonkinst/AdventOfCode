...
def main():
    ...
    for m in range(numMix):
        for i in range(n):
            ...
            # What if adding this timesCircled offset makes it overflow?
            # While it's inconsistent, keep adjusting the value until it is
            while (toMoveIndex + val + timesCircled) // n != timesCircled:
                timesCircled = (toMoveIndex + val + timesCircled) // n
            newIndex = (toMoveIndex + val + timesCircled) % n
        ...
        print(m + 1, n1, n2, n3, "TOTAL", n1 + n2 + n3)
        print()
        
        # If we're changing the instruction order
        # original = copy
        # copy = []
        # lookup = {}
        # for i in range(n):
