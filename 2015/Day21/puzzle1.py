...
def wins(boss, player):
    ...
    bossHealth = boss[0]
    playerTurns = playerHealth // bossDmg
    bossTurns = bossHealth // playerDmg
    ...
    while len(q) > 0:
        ...
        if wins(boss, player):
            if cost < minCost:
                minCost = cost
                minBuild = equipment
            # Adding something would only increase cost
            # This only works because we sorted by price earlier
        else:
