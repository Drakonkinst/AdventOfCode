...
def main():
    ...
    for name, module in nodes.items():
        if isinstance(module, Conjunction):
            #print("Module", name, "has", len(inputs[name]), "inputs:", inputs[name])
    ...
    while pushCount < maxPushes:
        ...
        while pulses:
            ...
            #print(sender, "-low->" if not isHigh else "-high->", receiver)
            if isHigh:
                numHigh += 1
            else:
                numLow += 1
    ...
    #print("CYCLE FOUND", pushCount, numHigh, numLow)
    pushesRemaining = maxPushes - pushCount
    if pushesRemaining > 0:
        numCycles = pushesRemaining // pushCount
        numLow *= (numCycles + 1)
        numHigh *= (numCycles + 1)
        pushesRemaining = pushesRemaining - (numCycles * pushCount)
        #print("CYCLE COMPUTED", numCycles, pushesRemaining)
    ...
    while pushesRemaining > 0:
        ...
        while pulses:
            ...
            if result is not None:
                for output in outputs[receiver]:
                    pulses.append((output, receiver, result))
    #print(numLow, numHigh, numLow * numHigh)
